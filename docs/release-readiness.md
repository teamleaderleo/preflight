...
- [ ] Decide whether the first beta waits for platform signing. Documented unsigned warnings are not
...
- [ ] Exercise the licensed game on real Windows and Linux installations. CI package builds and
  synthetic fixtures do not prove game integration.
...
- [ ] Make the first-run path one primary action: detect installation, explain required disk space,
...
- [ ] Preparation, profile switching, cleanup, and launch share ownership so stale concurrent state
  cannot be published or deleted.
...
The first public post should lead with the user outcome, the exact reviewed profile, one controlled
before/after result, the fail-open boundary, visible disk cost, and beta limitations. It should not
