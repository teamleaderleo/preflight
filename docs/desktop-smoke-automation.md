...
The injected JVM publishes `runtime-state.json` from a completed same-directory staged file. It
requests atomic replacement and falls back to ordinary staged replacement when the filesystem
provider reports atomic moves unsupported. Its PID and process start instant bind it to the same
...
Every injected game JVM publishes `runtime-process.json` from a completed same-directory staged
file before normal game initialization, requesting atomic replacement and falling back to ordinary
staged replacement when unsupported. The versioned record contains the JVM's PID, parent PID,
available process start instant, observation instant, and `running|stopped` state. A driver attaches
only by that PID after confirming the live process start instant matches the record. It never
resolves an application by display name, and a PID without a matching start instant isn't
...
The runner writes this document from a completed same-directory staged file, requesting atomic
replacement and falling back to ordinary staged replacement when unsupported. `failed` means an
assertion or action failed; `skipped` means a required driver capability or OS permission was
unavailable. A driver failure must not be reported as a game regression. Audio and visual
comparisons record their thresholds and reference identities in additive artifact fields rather
than hiding them in driver code.
...
SHA-256 itself, and publishes the final document from a completed same-directory staged file using
atomic replacement when supported and ordinary staged replacement otherwise. The output filename is
fixed so a driver can't redirect the write onto unrelated run evidence.
