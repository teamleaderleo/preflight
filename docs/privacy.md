...
Local maintenance keeps 10 launch reports—including the latest completed paired comparison when
available—and the 5 newest benchmark campaigns. It removes older Preflight evidence while the
desktop is idle. This is local deletion only; it makes no network request and doesn't touch
