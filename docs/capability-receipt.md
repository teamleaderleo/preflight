...
- Ordinary preparation and launch send no telemetry. Update checks use the fixed signed-update URL
  in the receipt. Support ZIPs require review and confirmation. The first beta does not send
  failed-run reports automatically. Sending requires the exact HTTPS intake origin shown in the
  receipt.
