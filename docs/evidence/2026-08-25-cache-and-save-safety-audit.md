...
resource-index, texture, classpath-index, and audio preparation commands. The resource-index command
uses it when no explicit `--output` export was named; the other commands apply it to their requested
prepared-data root. Each check happens before installation discovery, source scanning, storage
planning, or an audio child process. An existing symbolic root or non-directory root is refused; a
normal existing directory resolves to its canonical path, and a new real directory remains an allowed
destination.

This prevents a stale or replaced cache entry from redirecting preparation into an unrelated tree.
Regression coverage runs every public writer against a symbolic cache root containing an external
