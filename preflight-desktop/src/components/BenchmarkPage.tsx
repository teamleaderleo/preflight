      ...
      <section className="card benchmark-card">
        <div>
          <div className="heading-with-info">
            ...
            <InfoTip label="About the benchmark">Opens Starsector twice and times each launch at the main menu: first normally, then with Preflight. It only closes the game process started for the benchmark.</InfoTip>
          </div>
          <p>{isReady
              ...
              <small aria-live="polite">
                {benchmarkCopyState === "copied"
                  ...
                  : benchmarkCopyState === "error"
                    ...
                    : "Copies the two measured startup times with a short note about this installation."}
              </small>
            </div>
          ) : null}
          <small>The saved result keeps the game, Preflight, and runtime versions with the raw timings.</small>
        </section>
      ) : null}
...
function BenchmarkContext({ comparison }: { comparison: AutomationState["desktopBenchmarkComparison"] }) {
  ...
  return (
    <div className="benchmark-results__context" aria-label="Benchmark context">
      ...
      {runtime ? <span><strong>{compactNumber(runtime.fallbacks)}</strong> normal fallback{runtime.fallbacks === 1 ? "" : "s"}</span> : null}
      {runtime && runtime.failures > 0 ? <span className="benchmark-results__warning"><strong>{compactNumber(runtime.failures)}</strong> optimization issue{runtime.failures === 1 ? "" : "s"}</span> : null}
