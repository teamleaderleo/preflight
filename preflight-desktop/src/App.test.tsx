...
test("an unconfigured build keeps local export available and exposes no remote send action", async () => {
  const user = userEvent.setup();
...
test("a stale report-error preview stays local-only when the build is unconfigured", async () => {
  const user = userEvent.setup();
  const intake = vi.spyOn(bridge, "getReportIntakeStatus").mockResolvedValue({
    configured: false,
    origin: null,
