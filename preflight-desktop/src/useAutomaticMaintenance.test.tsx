...
test("prunes prepared profiles when free disk space is under the safety threshold", async () => {
  vi.spyOn(bridge, "isDesktopHost").mockReturnValue(true);
  vi.spyOn(bridge, "applyEvidenceCleanup").mockResolvedValue({
    format: "starsector-preflight-evidence-prune-v1",
    applied: true,
  ...
  const cachePlan: CacheCleanupPlan = {
    ...
    files: 10,
    reachableTextureBlobs: 20,
    reachablePreparedAudioBlobs: 2,
    refusals: [],
    groups: [],
    removals: [],
    removalsTruncated: false,
  ...
  const cleaned = vi.fn();
  const { rerender } = renderHook(({ freeBytes }) => useAutomaticMaintenance(true, 0, {
    game: "/Applications/Starsector",
    cacheBytes: 2 * 1024 * 1024 * 1024,
    freeBytes,
    onCacheCleaned: cleaned,
  }), { initialProps: { freeBytes: 10 * 1024 * 1024 * 1024 } });

  await act(async () => vi.advanceTimersByTimeAsync(1_500));
  expect(plan).not.toHaveBeenCalled();
  expect(apply).not.toHaveBeenCalled();

  rerender({ freeBytes: 3 * 1024 * 1024 * 1024 }); // Under 5 GiB safety threshold
  await act(async () => vi.advanceTimersByTimeAsync(1_500));
  expect(plan).toHaveBeenCalledWith("/Applications/Starsector");
  expect(apply).toHaveBeenCalledWith("/Applications/Starsector");
  expect(cleaned).toHaveBeenCalledTimes(1);
