...
test("persists a shared bounded ship view", () => {
  ...
  act(() => first.result.current.setView({ yaw: -0.7, pitch: 1.2 }));
  expect(second.result.current).toEqual(expect.objectContaining({ yaw: -0.7, pitch: 1.2 }));
  expect(JSON.parse(window.localStorage.getItem(INSTRUMENT_HULL_VIEW_STORAGE_KEY) ?? "null"))
    .toEqual({ yaw: -0.7, pitch: 1.2 });
});
...
test("repairs invalid values and resets mounted consumers", () => {
  ...
  act(() => result.current.setView({ yaw: 1, pitch: 1 }));
