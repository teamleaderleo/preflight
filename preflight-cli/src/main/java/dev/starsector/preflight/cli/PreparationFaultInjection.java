...
final class PreparationFaultInjection {
    ...
    static void afterLeaseAcquired() throws IOException {
        if (!enabled()) return;
        String configured = System.getProperty(PAUSE_MARKER_PROPERTY, "").trim();
        ...
        while (!Files.exists(release)) {
            if (Instant.now().isAfter(deadline)) {
                throw new IOException("Timed out at the injected post-lease preparation pause");
            }
            try {
                Thread.sleep(20);
            } catch (InterruptedException interrupted) {
                Thread.currentThread().interrupt();
                throw new IOException("Interrupted at the injected post-lease preparation pause", interrupted);
            }
        }
    }
