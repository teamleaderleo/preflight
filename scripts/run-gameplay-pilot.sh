...
if [[ -f "$OUT/adapter.json" ]]; then
    ...
    jq '{graphicsLibCompactReplay, janinoBytecodeCache, graphicsLibInsigniaManagerCache, magicLibPaintjob, campaignEntityIndex, deploymentIconCache, simOpponentSafety}' \
        "$OUT/adapter.json"
else
