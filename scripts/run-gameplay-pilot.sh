...
if [[ -f "$OUT/adapter.json" ]]; then
    ...
    jq '{graphicsLibCompactReplay, janinoBytecodeCache, graphicsLibInsigniaManagerCache, magicLibPaintjob, magicLibPaintjobNotification, stelnetMarketUpdater, campaignEntityIndex, deploymentIconCache, simOpponentSafety}' \
        "$OUT/adapter.json"
else
