...
run_ps=$(cat <<EOF
\$script = "$guest_repo\\scripts\\run-windows-startup-cohort.ps1"
\$args = '-NoProfile -ExecutionPolicy Bypass -File "' + \$script + '" -PreflightJar "$guest_jar" -Iterations $iterations -CooldownSeconds $cooldown -Conditions $condition -OptimizationPreset $preset$faction_priority_arg$startup_phase_arg$display_thread_texture_arg$display_thread_spec_store_arg$spec_store_texture_overlap_arg'
Set-ScheduledTask -TaskName "$task" -Action (New-ScheduledTaskAction -Execute 'powershell.exe' -Argument \$args) | Out-Null
