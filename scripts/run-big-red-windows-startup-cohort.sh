...
windows_oversized_merged_read_keys=false
...
Usage: run-big-red-windows-startup-cohort.sh [options]
  ...
  --windows-oversized-merged-read-keys  Cache valid oversized Windows merged-JSON requests
...
while (($#)); do
    case "$1" in
        ...
        --windows-oversized-merged-read-keys) windows_oversized_merged_read_keys=true; shift ;;
...
spec_store_texture_overlap_arg=""
[[ "$spec_store_texture_overlap" == true ]] && spec_store_texture_overlap_arg=" -WindowsSpecStoreTextureOverlap"
windows_oversized_merged_read_keys_arg=""
[[ "$windows_oversized_merged_read_keys" == true ]] && windows_oversized_merged_read_keys_arg=" -WindowsOversizedMergedReadKeys"
run_ps=$(cat <<EOF
\$script = "$guest_repo\\scripts\\run-windows-startup-cohort.ps1"
\$args = '-NoProfile -ExecutionPolicy Bypass -File "' + \$script + '" -PreflightJar "$guest_jar" -Iterations $iterations -CooldownSeconds $cooldown -Conditions $condition -OptimizationPreset $preset$faction_priority_arg$startup_phase_arg$display_thread_texture_arg$display_thread_spec_store_arg$spec_store_texture_overlap_arg$windows_oversized_merged_read_keys_arg'
Set-ScheduledTask -TaskName "$task" -Action (New-ScheduledTaskAction -Execute 'powershell.exe' -Argument \$args) | Out-Null
