...
| `run-startup-benchmark.sh --engine PATH [--engine-sha256 HEX]` | The same campaign against the engine a release candidate carries, rather than a checkout build. `PATH` is a `preflight.jar`, an installed `Preflight.app`, or any directory holding `engine/preflight.jar`; there is no fallback to `preflight-cli/target/preflight.jar`, so a candidate campaign cannot quietly measure checkout bytes. **A release claim needs this**; without it the session records itself as a checkout build. |
