...
| | |
|---|---|
| `prune_local_build_outputs.py` | Preview rebuildable Maven, Rust, frontend, UI-matrix, and package outputs across this repository's registered worktrees. The current worktree and anything less than 24 hours old are retained; the newest completed build set expires after 72 hours, and old generated output may be removed from dirty worktrees without touching their source changes. After committing a verified wave, `--retire-current` can include that clean worktree immediately. Pass `--apply` only after reviewing the plan. |
