...
| `starsector_gameplay_hotspots.py` | Rank sampled gameplay stacks, campaign and combat scored separately. Add `--allocations` to rank JFR's weighted allocation estimates by object class, leaf, and first non-JDK owner; repeat `--step paused-settled --step unpaused-settled` to use the exact scenario receipt windows and exclude setup churn. |
