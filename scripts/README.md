...
| `starsector_balance_analysis.py --output benchmark-results/balance/NAME` | Build a local paper-balance database from core plus the exact enabled-mod order. Emits hull, weapon, and fitted-variant rankings, multiple weight profiles, rank stability, comparable-peer Pareto dominance, provenance, and data-quality coverage. Full derived rows stay in ignored operator evidence; the script never edits or copies installed content. See the [model and limits](../docs/balance-analysis.md). |
