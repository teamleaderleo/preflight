...
or the physical cursor position. Its verification step rejects a run if external control, center,
or settled zoom differs, because Starsector's cursor-following combat camera otherwise changes the
rendered workload. Use it for controlled FPS comparisons after the profiled scenario has identified
a candidate. Profiler FPS belongs to discovery evidence, not to an optimization claim.
