...
| `run-startup-benchmark.sh --unattended [--conditions LIST] [--rounds N] [--cooldown-seconds N]` | The repeated campaign: conditions shuffled inside each round, resumable, and the report refuses a result below five runs per condition. Slow and unattended. **Use this to prove a change moved something**, not to look around. `--cache PATH --texture-storage compact` measures an explicitly prepared Compact cache without silently replacing it with Balanced. |
