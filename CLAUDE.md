...
The command always keeps the current worktree, recent output, and the newest completed build set.
It can remove old generated output from a dirty worktree, but never its source changes. Do not
preserve exact release evidence by abandoning it under
`target/` or `desktop-dist/`; move reviewed evidence to its owned artifact location and document its
identity.
