...
/**
 * Intrusive, opt-in proof that the live LWJGL Display context can move to one persistent renderer
 * thread and back. Unlike the rejected shared-Pbuffer probes, this uses the Display context itself.
 * It never intercepts or replaces a normal game texture.
 */
public final class DisplayThreadTextureProbeRuntime {
    static final String ENABLED_PROPERTY = "preflight.startup.displayThreadTextureProbe";
    ...
    private static final long INTERRUPT_GRACE_MILLIS = 1_000L;
    ...
    private DisplayThreadTextureProbeRuntime() {
    }
    ...
    /** Called once after the reviewed no-argument Display.update has left LWJGL's global lock. */
    public static void onDisplayBoundary() {
        ...
        if (!System.getProperty("os.name", "").toLowerCase(Locale.ROOT).startsWith("windows")) {
            ...
            problem = "Display-thread ownership proof is gated to Windows";
    ...
    static void executeProof(ProbeGlApi gl, long workerTimeoutMillis,
        ...
        try {
            ...
            displayDrawableIdentity = objectIdentity(gl.displayDrawable());
            ...
            result = new WorkerResult(gl, textureIds);
            Thread worker = new Thread(result, "Preflight-Display-Texture-Probe");
            ...
            if (worker.isAlive()) {
                ...
                problem = "Display ownership worker exceeded " + workerTimeoutMillis + " ms";
            ...
            restoreDisplay(gl);
            if (workerTimedOut) {
                throw new IllegalStateException("Display ownership worker exceeded bounded wait");
            }
            ...
            if (displayReleased && !displayRestored && workerTerminated) {
                try {
                    ...
                    if (problem == null) problem = describe(unwrap(restoreFailure));
                    status = "display-restore-failed";
                    transition("display-restore-failed");
                }
            }
            if ((!displayReleased || displayRestored) && (workerTerminated || result == null)) {
                try {
    ...
    private static void verify(ProbeGlApi gl, int textureId, int edge) throws Exception {
        ...
        for (int index = 0; index < actual.capacity(); index++) {
            ...
            if (actual.get(index) != expected) {
                throw new IllegalStateException("Display texture mismatch edge=" + edge
    ...
    private static ByteBuffer pixels(int edge) {
        ByteBuffer pixels = ByteBuffer.allocateDirect(Math.multiplyExact(edge * edge, 4));
        for (int index = 0; index < pixels.capacity(); index++) pixels.put(value(index, edge));
        return pixels.flip();
    }
    ...
    static synchronized Map<String, Object> telemetry() {
        ...
        result.put("workerDrawable", "live-display-context");
    ...
    private static final class WorkerResult implements Runnable {
        ...
        private WorkerResult(ProbeGlApi gl, int[] textureIds) {
            this.gl = gl;
        ...
        public void run() {
            ...
            try {
                ...
                current = true;
                workerCurrentAfterAcquire = gl.displayIsCurrent();
                ...
                texturesUploaded = 2;
                bytesUploaded = 4L * 4L * 4L + (long) LARGE_EDGE * LARGE_EDGE * 4L;
                ...
                if (current) {
                    try {
                        ...
                        workerCurrentAfterRelease = gl.displayIsCurrent();
    ...
    private static final class ReflectionGlApi implements ProbeGlApi {
        ...
        private ReflectionGlApi(Class<?> display, Class<?> gl11) throws ReflectiveOperationException {
        ...
        static ReflectionGlApi load(ClassLoader loader) throws ReflectiveOperationException {
            ...
            return new ReflectionGlApi(display, gl11);
        }
        ...
        @Override public void texImage2d(int target, int level, int internalFormat, int width,
                int height, int border, int format, int type, ByteBuffer pixels) throws Exception {
            glTexImage2d.invoke(null, target, level, internalFormat, width, height, border,
                    format, type, pixels);
        }
        @Override public void getTexImage(int target, int level, int format, int type,
        ...
        @Override public void drainErrors() throws Exception {
            for (int attempts = 0; attempts < 32 && getError() != GL_NO_ERROR; attempts++) {
                // Bound stale-error clearing so the proof starts with attributable state.
            }
        }
    }
}
